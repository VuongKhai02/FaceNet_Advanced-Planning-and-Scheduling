...
const InfoRow3 = ({ label, data, space }: any) => {
    return (
        <>
      ...
      <div style={{ marginLeft: space ? space : 105, marginTop: -20, display: "inline" }}>
        <span style={{ padding: 2, height: 30, maxWidth: 200 }}>{data}</span>
            ...
            <div style={{ padding: 2, marginTop: 20, height: 30, display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontWeight: "bold", width: "11.5%", display: "flex", justifyContent: "end" }}>
                    {label}
                    <span>: </span>
                </span>
                <span style={{ padding: 2, height: 30, width: "85%" }}>{data}</span>
            </div>
        </>
    );
};
