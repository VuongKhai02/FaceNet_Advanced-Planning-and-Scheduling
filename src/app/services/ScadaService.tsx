...
const getStageList = async () => {
    ...
    // let expiredTime = window.localStorage.getItem("scada_stage_list_time");
    // // console.log("get data from local storage first")
    // if (data && expiredTime && new Date().getTime() < JSON.parse(expiredTime)) {
    //   // console.log(data)
    ...
    if (response && response.status === 200) {
        ...
        window.localStorage.setItem("scada_stage_list_time", JSON.stringify(new Date().getTime() + 1000 * 60 * 5));
...
const getLineList = async () => {
    ...
    // let expiredTime = window.localStorage.getItem("scada_line_list_time");
    // // console.log("get data from local storage first")
    // if (data && expiredTime && new Date().getTime() < JSON.parse(expiredTime)) {
    //   // console.log(data)
    //   return JSON.parse(data);
    ...
    if (response && response.status === 200) {
        ...
        window.localStorage.setItem("scada_line_list_time", JSON.stringify(new Date().getTime() + 1000 * 60 * 5));
        return dataArrayReturn;
    }
    return null;
};
...
const getQuantityOut = async (token, assetId) => {
    ...
    let response = await axios.get(SCADA_URL + "/api/plugins/telemetry/ASSET/" + assetId + "/values/attributes/SERVER_SCOPE", { headers });
