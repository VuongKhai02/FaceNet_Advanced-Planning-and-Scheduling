...
import { BomVersionView } from "../../product/BomVersionView";
import { Coitt } from "../../../jmix/entities/Coitt";
import CreateWorkOrderByProduct from "./CreateWorkOrderByProduct";
import { Tooltip } from "devextreme-react/tooltip";
import notify from "devextreme/ui/notify";
import ProductSelectBoxTemplate from "./ProductSelectBoxTemplate";
...
import { CreateWorkOrderByProductPopup } from "../popup/CreateWorkOrderByProductPopup";
...
import { currentDateTime, customizeColor } from "../../../utils/utils";
import Keycloak from "../../../Keycloak";
import { ViewDocUrlButton } from "./ViewDocUrlButton";
...
const dataInstance = instance<ProductOrderItem>(ProductOrderItem.NAME, { view: "_base", loadImmediately: false });
...
export const ProductOrderItemTemplate = React.memo((props: any) => {
    const createWOByProdObj = new CreateWOByPOObject();
    ...
    const [listBom, setListBom] = useState<Citt1[]>([]);
    ...
    productOrderItemDs.filter = {
        conditions: [{ property: "productOrder", operator: "=", value: productOrderId }],
    };

    const loadProductOrderItem = () => {
        productOrderItemDs.load().then((res) => {
            setData(productOrderItemDs.items);
        });
    };
    const loadProduct = () => {};
        ...
        return (
            <>
                <div id={"buttonCreateWOByProduct" + row.data.id}>
                    <BomVersionButton row={row} showBomversionView={showCreateWOByProductPopup} icon={"plus"} />
                    <Tooltip
    ...
    const buttonViewBom = (row) => {
        return (
            <div id={"buttonViewBom" + row.data.id}>
                <BomVersionButton
                    row={row}
                    showBomversionView={showBomversionView}
                    disabled={!row.data.bomVersion}
                    hint='Xem Bom Version'
                />
                {/*<Tooltip*/}
                {/*  target={"#buttonViewBom" + row.data.id}*/}
                {/*  showEvent="dxhoverstart"*/}
                {/*  hideEvent="dxhoverend"*/}
                {/*  contentRender={() => {*/}
                {/*    return (*/}
    ...
    const renderPopup = () => {
        return <BomVersionView data={bomVersionProps} />;
    };

    const renderPopupCreateWO = () => {
        console.log("renderPopupCreateWO");
        return <CreateWorkOrderByProduct row={currentRow} closePopup={closePopup} productOrder={props.data.data} />;
    };
    ...
    const docUrlRender = (row) => {
        return <ViewDocUrlButton data={row} />;
    };
    ...
    const showBomversionView = async (row) => {
        ...
        let listBomVersion = collection<Citt1>(Citt1.NAME, {
            view: "_base",
            sort: "id.docEntry",
            loadImmediately: false,
        });
        await bomVersionDs.load().then((res) => {
            let bomVersion = bomVersionDs.items.pop();

            if (bomVersion && (bomVersion.uDocurl || bomVersion.uDocurl2)) {
                let productItem = new ProductOrderItem();
                if (bomVersion.uDocurl) {
                    productItem.sapUrl = bomVersion.uDocurl;
                    row.data.sapUrl = bomVersion.uDocurl;
                }
                if (bomVersion.uDocurl2) {
                    productItem.sapUrl2 = bomVersion.uDocurl2;
                    row.data.sapUrl2 = bomVersion.uDocurl2;
                }
                productItem.id = row.data.id;
                dataInstance.setItem(productItem);
                dataInstance.commit();
            }
            ...
            // if (bomVersion && bomVersion.id) {
            //   listBomVersion.filter = {
            //     conditions: [
            //       {property: 'docEntry', operator: "=", value: bomVersion.id},
    ...
    const onEdit = async (data) => {
        ...
        if (orderItem.bomVersion) {
            //get sap url
            bomVersionDs.filter = {
                conditions: [
                    { property: "uProno", operator: "=", value: orderItem.productCode },
                    { property: "uVersions", operator: "=", value: orderItem.bomVersion },
                ],
            };
            await bomVersionDs.load().then((res) => {
                let bomVersion = bomVersionDs.items.pop();
                ...
                if (bomVersion && (bomVersion.uDocurl || bomVersion.uDocurl2)) {
                    if (bomVersion.uDocurl) {
                        orderItem.sapUrl = bomVersion.uDocurl;
                    }
                    if (bomVersion.uDocurl2) {
                        orderItem.sapUrl2 = bomVersion.uDocurl2;
                    }
                }
            });
        }
        ...
        if (orderItem.quantity) {
            WorkOrderService.updateQuantityWoByPOAndProduct(data.oldData.productOrder, data.oldData.productCode, orderItem.quantity);
            // update po quantity
            let poDataInstance = instance<PlanningProductionOrder>(PlanningProductionOrder.NAME, {
                view: "_base",
                loadImmediately: false,
            });
            let oldQuantity = data.oldData.quantity;
            let changeQuantity = orderItem.quantity - oldQuantity;
            let po = props.data.data;
            po.quantity = po.quantity + changeQuantity;
            poDataInstance.setItem(po);
            poDataInstance.commit("edit");
        }
        BusinessLogService.save("EDIT_PRODUCT", orderItem, data.oldData.productOrder, data.oldData.productCode, startTime);
    };

    const onDelete = (data) => {
        console.log("on delete");
        let startTime = currentDateTime();
        productOrderItemDs.delete(data.data).then((res) => {
            notify(
                {
            ...
            let changeQuantity = data.data.quantity;
            let po = props.data.data;
            po.quantity = po.quantity - changeQuantity;
            poDataInstance.setItem(po);
            poDataInstance.commit("edit");

            BusinessLogService.save("REMOVE_PRODUCT", data.data, po.productOrderId, data.data.productCode, startTime);
        });
    };
    ...
    const onInsert = async (data) => {
        ...
        data.data.productOrder = props.data.data.productOrderId;
        let orderItem = data.data;
        orderItem.itemIndex = getIndexForNewRow();
        orderItem.createdAt = startTime;
        orderItem.createdBy = Keycloak.getUsername();
        ...
        dataInstance.commit().then((res) => {
            ...
            let changeQuantity = orderItem.quantity;
            let po = props.data.data;
            po.quantity = po.quantity + changeQuantity;
            poDataInstance.setItem(po);
            poDataInstance.commit("create");
            BusinessLogService.save("INSERT_PRODUCT", orderItem, po.productOrderId, orderItem.productCode, startTime);
        });
    };
        ...
        <div>
            <DataGrid
                ...
                <Column
                    ...
                    dataField={"coittId"}
                    editCellComponent={ProductSelectBoxTemplate}
                    visible={false}
                ...
                <Column
                    ...
                    caption={"View BOM"}
                    width={100}></Column>
                <Column cellRender={docUrlRender} alignment={"center"} caption={"View Hồ sơ"} width={140} renderAsync={true}></Column>
            ...
            <Popup2
                ...
                contentRender={renderPopup}></Popup2>
            <CreateWorkOrderByProductPopup createWOByProdObj={createWOByProdObj} />
            {/*<Popup2*/}
            {/*  // fullScreen={true}*/}
            {/*  width={"80%"}*/}
            {/*  height={"40%"}*/}
            {/*  showTitle={true}*/}
            {/*  // resizeEnabled={true}*/}
            {/*  title={"Khai báo Work Order"}*/}
            {/*  dragEnabled={true}*/}
            {/*  closeOnOutsideClick={true}*/}
            {/*  visible={popupVisible2}*/}
            {/*  onHiding={() => setPopupVisible2(false)}*/}
            {/*  contentRender={renderPopupCreateWO}*/}
            {/*>*/}
...
export default ProductOrderItemTemplate;
