...
export const TechProcedure: React.FC<TechProcedureProps> = observer(({ isOpen = false, setClose }) => {
    ...
    const handleMaJobChange = (e, rowKey) => {
        const newData = rowData.map((row) => (row.No === rowKey ? { ...row, MaJob: e.value } : row));
        setRowData(newData);
    };
    ...
    return (
        <>
            {isAddNewTechForm ? (
                <MaterialAndStructure isOpen={isAddNewTechForm} setClose={() => setIsAddNewTechForm(false)} />
            ) : (
