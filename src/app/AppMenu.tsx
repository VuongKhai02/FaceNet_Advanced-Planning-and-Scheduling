...
export const AppMenu = (props: AppMenuProps) => {
  ...
  const importManager = UserService.hasRole(["screen.Programing"]) ? (
    <SubMenuItem caption={"IMPORT MANAGER"} icon={<FileExcelOutlined/>}>
      ...
      <MenuItem caption={"2. PartNumber"} screenId='PartNumberImport' icon={<UploadOutlined/>}
                key={"screen.PartNumberImport"}/>
      ...
      />
      <MenuItem caption={"5. Programing"} screenId='ProgramingImport' icon={<UploadOutlined/>}
                key={"screen.ProgramingImport"}/>
      <MenuItem
  ...
  const employeeMenu = UserService.hasRole(["screen.Programing"]) ? (
    <SubMenuItem caption={"QUẢN LÝ THÔNG TIN"} icon={<BarsOutlined/>}>
      <MenuItem screenId={"EmployeeManager"} icon={<BarsOutlined/>} caption={"Quản lý nhân viên"}
                key={"EmployeeManager"}/>
      <MenuItem
  ...
  return (// @ts-ignore
    <VerticalMenu {...props}>
      ...
      {/*/>*/}
      <MenuItem screenId={"screen.DnlnvlManager"} icon={<BarsOutlined/>} caption={"QUẢN LÝ ĐỀ NGHỊ LĨNH"}
                key={"DnlnvlManager"}/>
      {/*<MenuItem*/}
